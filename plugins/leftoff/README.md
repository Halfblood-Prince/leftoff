...
The Go application lives at the repository root in `cmd/leftoff` and `internal/leftoff`. This directory is the plugin and skill packaging layer.
...
Release bundles include `leftoff.skill.zip`, a plugin-shaped archive with `.claude-plugin/`, `.codex-plugin/`, `skills/leftoff/SKILL.md`, self-contained skill scripts, launchers, and prebuilt platform binaries where available. Platform bundles also include a single prebuilt `leftoff` binary, including macOS Intel and Apple Silicon builds.
...
The setup flow downloads the platform release bundle with GitHub CLI, verifies GitHub artifact provenance, checks `SHA256SUMS`, and installs the binary under `~/.leftoff/bin/<os>_<arch>/`. It never uses `curl | sh`.
...
If Go is not installed, the source can still be inspected as plain text, but tests, formatting, and `go install github.com/Halfblood-Prince/leftoff/cmd/leftoff@latest` require a local Go toolchain.
